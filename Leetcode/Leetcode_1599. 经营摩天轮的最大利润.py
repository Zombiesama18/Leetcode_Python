...
def minOperationsMaxProfit(customers: List[int], boardingCost: int, runningCost: int) -> int:
    ...
    for i in range(len(customers)):
        ...
        turn_customer = min(4, waiting_customers)
        waiting_customers -= turn_customer
        current_benefit += turn_customer * boardingCost - runningCost
        if current_benefit > max_benefit:
            max_benefit = current_benefit
            result = operation
    if (waiting_customers // 4) * (4 * boardingCost - runningCost) + current_benefit > max_benefit:
        operation += waiting_customers // 4
        result = operation
        current_benefit += waiting_customers // 4
        max_benefit = current_benefit
    if (waiting_customers % 4) * boardingCost + current_benefit - runningCost > max_benefit:
        result = operation + 1
    return result


minOperationsMaxProfit([10,10,6,4,7], 3, 8)
minOperationsMaxProfit([5,0,0,0,0,30], 5, 5)
