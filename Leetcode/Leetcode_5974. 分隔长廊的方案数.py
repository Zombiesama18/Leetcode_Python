...
在下标 0     的左边和下标 n - 1     的右边 已经     分别各放了一个屏风。你还需要额外放置一些屏风。每一个位置     i - 1
和     i     之间（1 <= i <= n - 1），至多能放一个屏风。
...
def numberOfWays(corridor: str) -> int:
    ...
    for i in range(len(corridor)):
        if corridor[i] == 'S':
            positions.append(i)
    if len(positions) < 2 or len(positions) % 2 == 1:
        return 0
    subResult = 1
    for i in range(2, len(positions), 2):
        subResult *= positions[i] - positions[i - 1]
        subResult %= MOD
    result += subResult
    return result
...
items = ['S', 'P']
weights = [4, 6]
test = random.choices(items, weights, k=11)
numberOfWays(''.join(test))
