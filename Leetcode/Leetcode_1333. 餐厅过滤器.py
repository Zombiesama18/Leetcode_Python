...
class Solution:
    def filterRestaurants(self, restaurants: List[List[int]], veganFriendly: int, maxPrice: int, maxDistance: int) -> List[int]:
        result = []
        for ids, rating, veganFriend, price, distance in restaurants:
            if veganFriend >= veganFriendly and price <= maxPrice and distance <= maxDistance:
                heapq.heappush(result, (-rating, -ids))
        output = []
        while result:
            output.append(-heapq.heappop(result)[1])
        return output
