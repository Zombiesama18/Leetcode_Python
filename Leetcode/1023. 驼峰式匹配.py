"""
1023. 驼峰式匹配

如果我们可以将小写字母插入模式串 pattern 得到待查询项 query，那么待查询项与给定模式串匹配。（
我们可以在任何位置插入每个字符，也可以插入 0 个字符。）
给定待查询列表 queries，和模式串 pattern，返回由布尔值组成的答案列表 answer。只有在待查项 queries[i] 
与模式串 pattern 匹配时， answer[i] 才为 true，否则为 false。
"""
from typing import List


def camelMatch(queries: List[str], pattern: str) -> List[bool]:
    result = []
    for query in queries:
        query_ptr, pattern_ptr = 0, 0
        while query_ptr < len(query) and pattern_ptr < len(pattern):
            if query[query_ptr] == pattern[pattern_ptr]:
                query_ptr += 1
                pattern_ptr += 1
            elif query[query_ptr].isupper():
                break
            else:
                query_ptr += 1
        if pattern_ptr == len(pattern):
            start = query_ptr
            result.append(all([not query[query_ptr].isupper() for query_ptr in range(start, len(query))]))
        else:
            result.append(False)
    return result


camelMatch(queries = ["FooBar","FooBarTest","FootBall","FrameBuffer","ForceFeedBack"], pattern = "FB")
