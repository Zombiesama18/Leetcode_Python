...
# 其中 edges[i] = [ui, vi] 表示服务器 ui 和 vi 之间有一条信息线路，在 一秒 内它们之间可以传输 任意 数目的信息。再给你一个长度为 n 且下标从 0 开始的整数数组 patience 。
...
def networkBecomesIdle(edges: [[int]], patience: [int]) -> int:
    graph = collections.defaultdict(list)
    for source, target in edges:
        graph[target].append(source)
        graph[source].append(target)
    shortestDistance = [float('INF') for _ in range(len(patience))]
    shortestDistance[0] = 0
    nodeSet = {0}
    deque = collections.deque([0])
    while len(nodeSet) != len(patience):
        thisNode = deque.popleft()
        for nextNode in graph[thisNode]:
            nodeSet.add(nextNode)
            shortestDistance[nextNode] = min(shortestDistance[thisNode] + 1, shortestDistance[nextNode])
            deque.append(nextNode)
    result = 0
    flag = True
    for i in range(1, len(patience)):
        if 2 * shortestDistance[i] >= patience[i]:
            flag = False
        result = max(result, 4 * shortestDistance[i] - (2 * shortestDistance[i]) % patience[i])
